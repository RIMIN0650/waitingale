<template>
  <div class="hidden md:flex w-80 md:w-96 lg:w-[400px] flex-col bg-white border-l border-slate-200 shadow-xl z-10 h-full shrink-0 transition-all duration-300">
    
    <div class="p-5 border-b border-slate-100 bg-white sticky top-0 z-10">
      <div class="flex items-center justify-between mb-2">
        <div class="flex items-center gap-2">
          
          <button @click="$emit('close-list')" 
                  class="p-1.5 -ml-2 rounded-lg text-indigo-600 hover:bg-indigo-50 hover:text-indigo-700 transition"
                  title="목록 닫기">
            <i class="fa-solid fa-list-ul text-lg"></i>
          </button>

          <h2 class="text-lg font-black text-slate-900">병원 목록</h2>
        </div>
      ...
      <div class="bg-slate-50 rounded-xl border border-slate-100 mb-2 overflow-hidden">
        ...
        <button @click="isFavOpen = !isFavOpen" 
                class="w-full flex items-center justify-between p-3 bg-slate-50 hover:bg-slate-100 transition text-left">
          <h3 class="text-xs font-bold text-slate-700 flex items-center gap-1">
            <i class="fa-solid fa-heart text-rose-500"></i> 즐겨찾기
            <span v-if="favorites?.length" class="ml-1 text-[10px] text-slate-400 font-normal">
              ({{ favorites.length }})
            </span>
          </h3>
          
          <i class="fa-solid fa-chevron-down text-xs text-slate-400 transition-transform duration-300"
             :class="{ 'rotate-180': isFavOpen }"></i>
        </button>

        <transition name="slide-down">
          <div v-show="isFavOpen" class="px-3 pb-3">
            <div v-if="favorites?.length" class="space-y-1 pt-1">
              <div v-for="fav in favorites" :key="fav.id" 
                ...
                <button @click.stop="$emit('toggle-favorite', fav)" class="text-slate-300 hover:text-rose-500 p-1">
                   <i class="fa-solid fa-xmark text-xs"></i>
                </button>
              </div>
            </div>
            <p v-else class="text-xs text-slate-400 py-2 text-center">즐겨찾기한 병원이 없습니다.</p>
          </div>
        </transition>
      </div>
    </div>
...
<script setup>
  import { ref } from 'vue';
// props가 없을 때를 대비해 기본값([] 빈 배열) 설정
const props = defineProps({
  hospitals: { type: Array, default: () => [] },
  favorites: { type: Array, default: () => [] }
});
defineEmits(['item-click', 'toggle-favorite', 'close-list']);
